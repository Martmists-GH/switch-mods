...
void registerLuaUtils(lua_State *L) {
    ...
    lua_pushcfunction(L, reinterpret_cast<lua_CFunction>(luaToStringContent));
    lua_setfield(L, -2, "_BufString");
...
void lua_hooks() {
    ...
    LuaPanicHookInline1.installAtPtr(hxlua::State::OnPanicInline1);
    LuaAbortHook.installAtPtr(hxlua::AbortApi::Abort);
